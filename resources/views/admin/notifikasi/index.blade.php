...
<div class="header-card">
    <div>
        <h4>📢 Notifikasi System</h4>
        <p>Kelola notifikasi email dan popup untuk semua user</p>
    </div>
    <div>
        ...
        @else
            ...
            <div class="notif-item">
                <div>
                    <h6>{{ $n->judul }}</h6>
                    <p>{{ $n->pesan }}</p>
                    <span class="notif-status status-{{ $n->status }}">
                        {{ ucfirst(str_replace('_', ' ', $n->status)) }}
                    </span>
                </div>
                <div class="notif-actions">
                    @if($n->status === 'belum_dibaca')
                    <form action="{{ route('admin.notifikasi.read', $n->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
